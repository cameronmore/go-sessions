...
func RegisterHandler(secret string) (sessionId string, cookie *http.Cookie) {
	sessionId = newSessionId()
	signesSessionId := signSessionId(sessionId, secret)
	cookie = newCookie(signesSessionId)
	return
}

// Handles the login of a user by making a new cookie
func LoginHandler(secret string) (string, *http.Cookie) {
	return RegisterHandler(secret)
}
